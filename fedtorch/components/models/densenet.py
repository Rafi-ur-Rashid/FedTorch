...
class DenseNet(nn.Module):
    def __init__(self, dataset_config,
        ...
        for p in ['num_classes', 'type']:
                    if not hasattr(dataset_config, p):
                        raise ValueError("'{}' should be specified in the dataset config for a DenseNet model!".format(p))
        # determine some fundamental configurations.
        self.num_classes = dataset_config.num_classes
        is_small_inputs = 'imagenet' not in dataset_config.type
