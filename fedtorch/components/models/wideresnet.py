...
class WideResNet(nn.Module):
    def __init__(self, dataset_config, net_depth, widen_factor, drop_rate):
        super(WideResNet, self).__init__()
        for p in [ 'num_classes']:
            if not hasattr(dataset_config, p):
                raise ValueError("'{}' should be specified in the dataset config for a WideResNet model!".format(p))
        if not dataset_config.name in ['cifar10', 'cifar100', 'svhn', 'downsampled_imagenet']:
            raise NotImplementedError(f"The WideResNet model is not configured for {dataset_config.name} dataset")
