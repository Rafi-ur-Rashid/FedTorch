...
from fedtorch.components.datasets.utils import TqdmUpTo
...
def epsilon(root, split, num_classes=2, dimension=[2000]):
    return LibSVMDataset(root,'epsilon',split)

@DATASET.register_module()
def rcv1(root, split, num_classes=2, dimension=[47236]):
    return LibSVMDataset(root,'rcv1',split)

@DATASET.register_module()
def url(root, split, num_classes=2, dimension=[3231961]):
    return LibSVMDataset(root,'url',split)

@DATASET.register_module()
def higgs(root, split, num_classes=2, dimension=[28]):
    return LibSVMDataset(root,'higgs',split)

@DATASET.register_module()
def MSD(root, split, num_classes=1, dimension=[90]):
    return LibSVMDataset(root,'MSD',split)
