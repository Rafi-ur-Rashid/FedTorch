...
from fedtorch.components.datasets.utils import load_shakespeare, load_emnist
from fedtorch.components.dataset_builder import DATASET

@DATASET.register_module()
def emnist(root, split, client_id, download, only_digits=True, num_classes=10, dimension=[28,28]):
    return _get_emnist(root, split, client_id, download, only_digits)

@DATASET.register_module()
def shakespeare(root, split, client_id, download, batch_size=2, seq_len=50, num_classes=90, dimension=[90]):
    _get_shakespeare(root, split, client_id, download, batch_size, seq_len)

@DATASET.register_module()
def synthetic(root, split, client_id, num_tasks, alpha, beta, regression=False,
               num_dim=60, num_classes=2,  dimension=[60],
               min_num=500, max_num=1000):
    return Synthetic(root, split=split, client_id=client_id,
                    num_tasks=num_tasks, alpha=alpha, 
                    beta=beta, regression=regression, num_dim=num_dim,
                    num_classes=num_classes, min_num=min_num, max_num=max_num)

@DATASET.register_module()
def synthetic_polar(root, split, client_id, num_tasks, alpha, beta, regression=False,
               num_dim=60, num_classes=2,  dimension=[60],
               min_num=500, max_num=1000):
    return Synthetic_polar(root, split=split, client_id=client_id,
                    num_tasks=num_tasks, alpha=alpha, 
                    beta=beta, regression=regression, num_dim=num_dim,
                    num_classes=num_classes, min_num=min_num, max_num=max_num)

@DATASET.register_module()
def cifar10_federated(root, split, client_id, download, num_classes=10, dimension=[3,32,32]):
    return CIFAR10_Federated(root, split, client_id, download)

def _get_emnist(root, split, client_id, download, only_digits=True):
    if split == 'train':
...
def _get_shakespeare(root, split, client_id, download, batch_size=2, seq_len=50):
    if split == 'train':
...
class Synthetic(Dataset):
    ...
    def __init__(self,root, split='train', client_id=None, num_tasks=50, alpha=0.0, beta=0.0,
                 generate=True, regression=False, num_dim=60, dim_max=60, num_classes=2,
                 min_num=500, max_num=1000):
        self.root= root
        self.split=split
        self.client_id = client_id
        ...
        if generate:
            if self.num_dim == 0:
                self.num_dim = torch.randint(10,dim_max,[1]).item()
            self.generate_synthetic(num_dim=self.num_dim,
                                    num_classes=self.num_classes,
                                    min_num_samples=self.min_num,
                                    max_num_samples=self.max_num)
    ...
    def get_num_samples(self, min_num=500, max_num=1000):
        num_samples = np.random.lognormal(3, 4, (self.num_tasks)).astype(int)
    ...
    def _generate_y(self, x, num_classes):
        ...
        m_loc = random.choice([0.1,-.1])
        loc = np.random.normal(loc=m_loc, scale=self.alpha, size=None)
...
class Synthetic_polar(Synthetic):
    def get_all_tasks(self,num_samples, num_dim, num_classes, Sigma):
        self.w =  np.random.normal(loc=0.1, scale=0.1, size=(num_dim, 1))
        self.mu = np.random.normal(loc=0.2, scale=0.1, size=num_dim)
        n = len(num_samples)
        tasks = []
        for i,ns in enumerate(num_samples):
            if i>=n//2:
                self.w = -self.w
                self.mu = np.random.normal(loc=-0.2, scale=0.1, size=num_dim)
            tasks.append(self.get_task(ns, num_dim, num_classes, Sigma))
        import random
    ...
    def _generate_x(self, num_samples,num_dim,Sigma):
        samples = np.random.multivariate_normal(
            mean=self.mu, cov=Sigma, size=num_samples)

        return samples

    def _generate_y(self, x, num_classes):
        out = np.matmul(x, self.w) #+ np.random.normal(loc=0, scale=0.1, size=(num_samples, 1))

        y = np.squeeze((out >= 0 ).astype(float))
...
class CIFAR10_Federated(Dataset):
    ...
    def __init__(self,root, split='train', client_id=None, download=False):
        self.root= root
        self.split=split
        self.client_id = client_id
        self.num_clients = 50 
        ...
        if download:
            raise ValueError("Download is not supported for CIFAR10_Federated dataset")

        if self.split in ['train','val']:
            path = os.path.join(self.root, self.split, 'cifar10_client_{}.pt'.format(self.client_id))
        else:
            path = os.path.join(self.root, 'cifar10_federated_test.pt')
    ...
    def _check_exists(self):
        if self.split == 'test':
            return os.path.exists(os.path.join(self.root, "cifar10_federated_test.pt"))
        if not os.path.exists(os.path.join(self.root, self.split)):
            return False
        num_data_files = len(glob.glob(os.path.join(self.root,self.split,  "cifar10*.pt")))
        if self.split in ['train', 'val']:
            chk_num_data_files = num_data_files == self.num_clients
        else:
            chk_num_data_files = num_data_files == 1
