...
def scaffold_aggregation(cfg, model_server, model_client, model_server_control, model_client_control,
    ...
    for cccp, ccp, scp, cp, sp in zip(model_client_control_copy.parameters(), model_client_control.parameters(), model_server_control.parameters(), model_client.parameters(), model_server.parameters()):
        ...
        gather_list = [torch.ones_like(t) for _ in range(num_online_clients)] if cfg.graph.rank == 0 else None
        ...
        scp.data -= d[1] * (len(online_clients) / cfg.graph.n_nodes)
        cp.grad.data = t[0]
        # Update server control variate
        scp.data -= t[1] * (len(online_clients) / cfg.graph.n_nodes)
    ...
    model_client_control = deepcopy_model(cfg, model_client_control_copy)
