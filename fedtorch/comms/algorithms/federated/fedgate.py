...
def fedgate_aggregation(cfg, model_server, model_client, model_delta, model_memory, 
    ...
    for server_param, client_param, delta_param, memory_param in zip(model_server.parameters(), model_client.parameters(), model_delta.parameters(), model_memory.parameters()):
        ...
        elif cfg.federated.compressed:
            g = client_param.grad.data + memory_param.data * rank_weight
            grad_v, grad_i, grad_s = compress_tensor(g, r=cfg.federated.compressed_ratio, comp_type='topk')
            gather_list_value = [torch.ones_like(grad_v) for _ in range(num_online_clients)] if cfg.graph.rank == 0 else None
            gather_list_index = [torch.ones_like(grad_i) for _ in range(num_online_clients)] if cfg.graph.rank == 0 else None
        ...
        if (0 in online_clients) or (cfg.graph.rank != 0):
