...
def train_and_validate_federated_drfa(client):
    ...
    if client.cfg.graph.rank == 0:
        ...
        lambda_vector = torch.stack(gather_list_size) / client.cfg.data.train_dataset_size
    else:
        dist.gather(torch.tensor(client.cfg.data.num_samples_per_epoch,  dtype=torch.float32), dst=0)
        lambda_vector = torch.tensor([1/client.cfg.graph.n_nodes] * client.cfg.graph.n_nodes)
    ...
    for n_c in range(client.cfg.federated.num_comms):
        ...
        client.cfg.federated.drfa_gamma *= 0.9
        if client.cfg.graph.rank in online_clients_server:
            ...
            k = torch.randint(low=1,high=client.cfg.training.local_step,size=(1,))
           ...
           # Validate the local models befor sync
            ...
            if client.cfg.federated.type == 'fedgate':
                client.model_server, client.model_delta = fedgate_aggregation(client.cfg, client.model_server, client.model, 
                                                                              ...
                                                                              lambda_weight=client.lambda_vector[client.cfg.graph.rank].item())
            elif client.cfg.federated.type == 'scaffold':
            ...
            client.kth_model = aggregate_models_virtual(client.cfg, client.kth_model, online_clients_group, online_clients)
        ...
        online_clients_lambda = set_online_clients(client.cfg)
        ...
        if client.cfg.graph.rank in online_clients_server_lambda:
            st = time.time()
            client.kth_model = distribute_model_server(client.kth_model, online_clients_group_lambda, src=0)
            client.cfg.comm_time[-1] += time.time() - st
            loss = torch.tensor(0.0)

            if client.cfg.graph.rank in online_clients_lambda:
                for _input, _target in client.train_loader:
                    _input, _target = load_data_batch(client.cfg, _input, _target, tracker)
                    # Skip batches with one sample because of BatchNorm issue in some models!
                    if _input.size(0)==1:
                    ...
                    break
            loss_tensor_online = loss_gather(client.cfg, torch.tensor(loss.item()), 
            ...
            if client.cfg.graph.rank == 0:
                ...
                loss_tensor[sorted(online_clients_server_lambda)] = loss_tensor_online * (client.cfg.graph.n_nodes / num_online_clients)
                # Dual update
                client.lambda_vector += client.cfg.federated.drfa_gamma * client.cfg.training.local_step * loss_tensor
